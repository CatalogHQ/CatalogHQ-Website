...
function SidebarTrigger({
  ...
  const label = isMobile ? "Menu" : open ? "Hide menu" : "Show menu"
  ...
  return (
    <Button
      ...
      className={cn(
        showLabel ? "gap-2 text-gray-600 hover:text-gray-900" : "size-7",
        className
      )}
      onClick={(event) => {
        onClick?.(event)
      ...
      {...props}
    >
      <PanelLeftIcon className={showLabel ? "h-4 w-4" : undefined} />
      {showLabel ? (
        <span>{label}</span>
      ) : (
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(() => {
    ...
    for (let index = 0; index < skeletonId.length; index += 1) {
      hash = skeletonId.charCodeAt(index) + ((hash << 5) - hash)
    }
